...
> {
  ...
  _parseSlow(
    ...
    // this is just a default implementation that maintains the backwards compatibility
    // to with the internal _parse prior the performance updates. It'll be overrided in
    // all built-in ZodType sub-classes
  ...
  constructor(def: Def) {
    ...
    this._fast =
      this._parseFast !== ZodType.prototype._parseFast &&
      this._parse === ZodType.prototype._parse;
  }
...
export class ZodString extends ZodType<string, ZodStringDef> {
  _parseFast(
    ...
    if (this._def.minLength !== null) {
      if (data.length < this._def.minLength.value) {
        invalid = true;
        ctx.addIssue(data, {
    ...
    if (this._def.maxLength !== null) {
      if (data.length > this._def.maxLength.value) {
        invalid = true;
        ctx.addIssue(data, {
    ...
    if (this._def.isEmail && !emailRegex.test(data)) {
      invalid = true;
      ctx.addIssue(data, {
        validation: "email",
    ...
    if (this._def.isUUID && !uuidRegex.test(data)) {
      invalid = true;
      ctx.addIssue(data, {
        validation: "uuid",
        code: ZodIssueCode.invalid_string,
...
export class ZodNumber extends ZodType<number, ZodNumberDef> {
  _parseFast(
    ...
    if (this._def.minimum) {
      ...
      const tooSmall = MIN.inclusive ? data < MIN.value : data <= MIN.value;
      if (tooSmall) {
        invalid = true;
        ctx.addIssue(data, {
          code: ZodIssueCode.too_small,
    ...
    if (this._def.maximum) {
      ...
      const tooBig = MAX.inclusive ? data > MAX.value : data >= MAX.value;
      if (tooBig) {
        invalid = true;
        ctx.addIssue(data, {
          code: ZodIssueCode.too_big,
